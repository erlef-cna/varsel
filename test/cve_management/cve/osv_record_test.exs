...
defmodule CveManagement.CVE.OsvRecordTest do
  ...
  setup do
    Application.put_env(:cve_management, :hex_stub_packages, %{
  ...
  defp backdate_sync(hours) do
    ...
    CveManagement.Repo.update_all("osv_records", set: [synced_at: stale])
  end
  ...
  describe "sync trigger" do
    ...
    test "leaves the OSV record untouched while the parent is pending_update" do
      ...
      CveManagement.Repo.update_all("cve_records", set: [state: "pending_update"])
      backdate_sync(25)
  ...
  defp stub_mitre_reject do
    Req.Test.stub(CveManagement.CVE.MitreCveApi, fn conn ->
