...
defmodule Varsel.CVE.OsvConverter do
  @moduledoc """
  ...
  `Varsel.CVE.OsvRecord`: the `modified` timestamp advances only
  ...
  defp version_info?(item, version_type) do
    ...
    Enum.any?(versions, &affected_of_type?(&1, version_type)) or
      (item["defaultStatus"] == "affected" and
         (versions == [] or Enum.any?(versions, &(&1["versionType"] == version_type))))
  end

  # Under an `unaffected` or `unknown` default, ranges come from the affected
  # rows alone: an explicit unaffected row restates a fix an affected row's
  # upper bound already carries, and an unknown row has no OSV representation.
  # Injecting the schema-required `introduced` event into either would claim an
  # affected span the record never made.
  ...
  defp semver_ranges(item) do
    if item["defaultStatus"] == "affected" do
      ...
      item
      |> Map.get("versions", [])
      |> Enum.filter(&affected_of_type?(&1, "semver"))
      |> Enum.map(&semver_range/1)
    end
  end

  # An `affected` default says every version is affected unless a row says
  # otherwise, which the schema states as its own "unfixed vulnerability" shape:
  # a range opening at `0`, carrying a `fixed` event per span the record calls
  # safe.
  defp default_affected_events(item, version_type, type) do
    rows = for row <- Map.get(item, "versions", []), row["versionType"] == version_type, do: row
    ...
    spans =
      for row <- rows,
          ...
          do: %{"fixed" => clean_version(version, type)}
  ...
  defp change_event(%{"status" => "affected", "at" => at}, type) when is_binary(at),
    do: [%{"introduced" => clean_version(at, type)}]
