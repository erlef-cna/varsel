%% SPDX-FileCopyrightText: 2026 Ericsson AB
%%
%% SPDX-License-Identifier: Apache-2.0
%%
%% Vendored verbatim from erlang/otp `lib/runtime_tools/src/versions.erl` as merged
%% by PR #11556, at commit 41ba1f4ca258f2947249a33b9a3793e5c58f18f8, with two edits:
%% the module renamed to `varsel_versions`, and the `-moduledoc` stripped of the
%% `@OTP-20352@` build placeholder and the `e:system:` doc refs that only resolve
%% inside OTP.
%%
%% Delete this file and switch the callers to `:versions` once the module ships in
%% runtime_tools; `Varsel.Cases.Reachability.OTPVersion` is the only caller.

-module(varsel_versions).
-moduledoc """
...
See the OTP Versions Scheme for
...
order between versions in the OTP
